...
export async function saveProduct(product) {
  // Generate id if missing before writing to DB or Supabase
  if (!product.id || typeof product.id !== 'string' || !product.id.trim()) {
    const slug = (product.title || 'product')
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/(^-|-$)/g, '');
    product.id = `prod-${slug}-${Date.now().toString().slice(-6)}`;
  } else {
...
export async function saveDiamond(diamond) {
  // Generate id if missing before writing to DB or Supabase
  if (!diamond.id || typeof diamond.id !== 'string' || !diamond.id.trim()) {
    const shapeStr = (diamond.shape || 'diamond').toLowerCase();
    const caratNum = Math.floor((parseFloat(diamond.carat) || 1) * 100);
    diamond.id = `dia-${shapeStr}-${caratNum}-${Date.now().toString().slice(-4)}`;
  } else {
    diamond.id = diamond.id.trim();
  }
  ...
  await writeJson('diamonds.json', diamonds); // wait! let's check writeJson parameter here!
  return diamond;
}
