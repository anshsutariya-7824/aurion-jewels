  ...
  return (
    <div className="flex flex-col space-y-8 sm:space-y-12 w-full max-w-full">
          ...
          {categories.map((cat) => {
            ...
            return (
              ...
              >
                <span>{cat}</span>
                {isSelected && (
                  <span className="ml-1.5 text-brand-gold text-[9px] sm:text-[10px]">✦</span>
                )}
              </button>
            );
          })}
      ...
      {filteredProducts.length === 0 ? (
        <div className="text-center py-16 sm:py-20 bg-brand-cream/50 border border-brand-charcoal/10 rounded-xl px-4">
          <p className="font-display italic text-lg sm:text-xl text-brand-slate">No masterpieces found in this collection.</p>
        </div>
      ) : (
              ...
              >
                ...
                <Link href={`/collections/${product.id}`} className="relative aspect-[4/4] w-full overflow-hidden bg-brand-sand/40 block">
                  ...
                  <div className="absolute top-3 left-3 sm:top-4 sm:left-4 z-10 flex items-center space-x-2">
                    <span className="bg-brand-charcoal/85 text-brand-cream backdrop-blur-md text-[9px] uppercase tracking-widest font-semibold px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full border border-brand-gold/30 shadow-xs">
                      {product.category}
                    </span>
                  </div>

                  <div className="absolute top-3 right-3 sm:top-4 sm:right-4 z-10">
                    <span className="bg-brand-gold/90 text-white text-[9px] uppercase tracking-widest font-bold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full shadow-xs">
                      MOQ: {product.moq || 1}
                    </span>
                  </div>

                  {/* Quick Hover CTA Banner (Desktop & Mobile) */}
                  <div className="absolute inset-x-0 bottom-0 p-3 sm:p-4 bg-gradient-to-t from-brand-charcoal/80 via-brand-charcoal/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center">
                    <span className="bg-white/95 text-brand-charcoal hover:bg-brand-gold hover:text-white text-[9px] sm:text-[10px] uppercase tracking-widest font-bold py-2 px-5 sm:py-2.5 sm:px-6 rounded-full shadow-md transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
                ...
                {/* Content Info Box */}
                <div className="p-4 sm:p-6 flex flex-col flex-grow space-y-2.5 sm:space-y-3 bg-gradient-to-b from-white to-brand-cream/20">
                  <h3 className="font-display text-lg sm:text-xl text-brand-charcoal font-medium group-hover:text-brand-gold transition-colors duration-300 leading-snug">
                  ...
                  <div className="pt-3 sm:pt-4 mt-auto border-t border-brand-charcoal/10 flex items-center justify-between">
                    <div>
                      <span className="text-[9px] sm:text-[10px] uppercase tracking-wider text-brand-slate/70 block">
                        Wholesale FOB
                      </span>
                    ...
                    <Link
                      ...
                      className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-brand-sand hover:bg-brand-gold hover:text-white text-brand-charcoal flex items-center justify-center transition-all duration-300 shadow-xs hover:shadow-md shrink-0"
